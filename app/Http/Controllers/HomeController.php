    ...
    public function index()
    {
        ...
        if (! empty($fy_sells_by_location_data)) {
            foreach ($fy_sells_by_location_data as $location_sell) {
                $sells_chart_2->dataset($location_sell['loc_label'], 'bar', $location_sell['values']);
            }
        }
        if (count($all_locations) > 1) {
            $sells_chart_2->dataset(__('report.all_locations'), 'bar', $values);
        }
    ...
    public function attachMediasToGivenModel(Request $request)
    {
        if ($request->ajax()) {
            try {
                ...
                \Log::emergency('File:'.$e->getFile().'bar:'.$e->getbar().'Message:'.$e->getMessage());
